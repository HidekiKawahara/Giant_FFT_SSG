...
if transitionStartFreq > 0
    ...
    normalizedTransitionFreqs = (positiveFreqs(is_in_transition) ...
        - transitionStartFreq) / options.transitionWidthHz;

    % Apply a half-cosine window shape to the transition band
    %shaper(is_in_transition) = 0.5 + 0.5 * cos(normalizedTransitionFreqs * pi);
    shaper = (1-exp(-(sqrt(-log(0.5))*(positiveFreqs-targetNyquist)/options.transitionWidthHz).^2));
end
